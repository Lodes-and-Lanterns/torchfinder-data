 ...
 *
 * Parses a Forgejo issue body (from an issue form), validates the content,
 * writes files, runs the build validator, and creates a branch + PR.
 *
 * The issue itself is fetched by number, so the script works from a plain
 * workflow_dispatch with no issue event payload. The issue's type is derived
 * from its labels ("new-entry" | "update-entry" | "remove-entry").
 *
 * Required env vars:
 *   GITHUB_TOKEN        Codeberg access token with repository write access
 *   GITHUB_REPOSITORY   "owner/repo" (set automatically by Forgejo Actions)
 *   ISSUE_NUMBER        issue number (string)
 *
 * Optional env vars:
 *   FORGEJO_API_URL     API base URL; defaults to https://codeberg.org/api/v1
 *
 * Run with:
...
const API_BASE = Deno.env.get("FORGEJO_API_URL") ??
...
async function fetchIssue(): Promise<ForgejoIssue> {
  const [owner, repo] = GITHUB_REPOSITORY.split("/");
  const res = await fetch(
    `${API_BASE}/repos/${owner}/${repo}/issues/${ISSUE_NUMBER}`,
    { headers: { Authorization: `token ${GITHUB_TOKEN}` } },
  );
  if (!res.ok) {
    console.error(
      `Failed to fetch issue #${ISSUE_NUMBER}: HTTP ${res.status}`,
    );
    Deno.exit(1);
  }
  return await res.json() as ForgejoIssue;
}

const TYPE_LABELS = ["new-entry", "update-entry", "remove-entry"];

const issue = await fetchIssue();
const ISSUE_BODY = issue.body ?? "";
const ISSUE_TITLE = issue.title ?? "";
const ISSUE_AUTHOR = issue.user?.login ?? "";
const ISSUE_TYPE = issue.labels
...
/** Forgejo attaches labels by numeric id, so names resolve through the repo's
    label list; missing labels are created on the fly. */
async function resolveLabelId(name: string): Promise<number | null> {
  const [owner, repo] = GITHUB_REPOSITORY.split("/");
  const listRes = await fetch(
    `${API_BASE}/repos/${owner}/${repo}/labels?limit=100`,
    { headers: apiHeaders() },
  );
  if (!listRes.ok) {
  ...
  const labels = await listRes.json() as { id: number; name: string }[];
  const found = labels.find((label) => label.name === name);
  if (found) return found.id;

  const createRes = await fetch(`${API_BASE}/repos/${owner}/${repo}/labels`, {
    method: "POST",
    headers: apiHeaders(),
    body: JSON.stringify({ name, color: "#cccccc" }),
  });
  if (!createRes.ok) {
    console.error(`Failed to create label "${name}":`, await createRes.text());
    return null;
  }
...
async function forgejoAddLabel(
  issueNumber: string,
  label: string,
): Promise<void> {
  ...
  const [owner, repo] = GITHUB_REPOSITORY.split("/");
  const url = `${API_BASE}/repos/${owner}/${repo}/issues/${issueNumber}/labels`;
  const res = await fetch(url, {
    method: "POST",
    headers: apiHeaders(),
...
async function forgejoCreatePR(params: {
  title: string;
  body: string;
  head: string;
  base: string;
...
async function gitSetup(): Promise<void> {
  ...
  await run(["git", "config", "user.email", "torchfinder-bot@noreply.codeberg.org"]);
}
...
async function handleNewEntry(fields: Map<string, string>): Promise<void> {
  ...
  const prUrl = await forgejoCreatePR({
  ...
  await forgejoComment(ISSUE_NUMBER, `PR created: ${prUrl}`);
  await forgejoAddLabel(ISSUE_NUMBER, "bot:pr-created");
...
switch (ISSUE_TYPE) {
  ...
  default:
    await failWithComment(
      `No recognized type label on this issue. Expected one of: ${
        TYPE_LABELS.map((name) => `\`${name}\``).join(", ")
      }.`,
    );
}
